...
<section class="hero">
    <div class="hero-content">
        ...
        <div class="hero-cta">
            <a href="{{ route('shop') }}" class="btn btn-gold" style="font-size:1rem;padding:.9rem 2.5rem;">Shop Now <i class="fas fa-arrow-right"></i></a>
    ...
    <div style="background-image: url( {{ asset('storage/products/banner.jpeg') }}); background-repeat: no-repeat; background-position: center; background-size: cover;">
            ...
            <a href="{{ route('shop', ['category' => $cat->slug]) }}" class="cat-card">
                <div class="cat-icon">
                    @if($cat->category_image)
                        <img src="{{ asset('storage/'.$cat->category_image) }}" alt="{{ $cat->category_name }}" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">
                    @else
...
<section class="products-section">
    <div class="container">
                ...
                <div class="product-card-body">
                    ...
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-top:.5rem;">
                        ...
                        <div class="stars">
                            ...
                            @for($i=1;$i<=5;$i++)<i class="fas fa-star{{ $i > $r ? ' empty' : '' }}" style="{{ $i > $r ? 'color:#ddd' : '' }}"></i>@endfor
                        </div>
                    </div>
                    <a href="{{ route('shop.show', $product->product_id) }}" class="btn btn-outline" style="width:100%;justify-content:center;margin-top:.75rem;">View Product</a>
                </div>
            </div>
            @empty
        ...
        <div style="text-align:center;margin-top:3rem;">
            <a href="{{ route('shop') }}" class="btn btn-primary" style="padding:.9rem 2.5rem;">View All Products <i class="fas fa-arrow-right"></i></a>
        </div>
    </div>
</section>
...
<section class="banner-section">
    ...
    <a href="{{ route('shop') }}" class="btn btn-gold" style="font-size:1rem;padding:.9rem 2.5rem;">Shop Now</a>
</section>
...
<section class="blog-section">
    <div class="container">
        ...
        <div class="blog-grid">
            ...
            <a href="{{ route('blog.show', $blog->blog_detail_id) }}" class="blog-card">
                ...
                <div class="blog-card-body">
                    <div class="blog-card-date">{{ $blog->created_at->format('M d, Y') }} · {{ $blog->blog->author_name ?? '' }}</div>
                    <h3 class="blog-card-title">{{ $blog->title }}</h3>
                    <p class="blog-card-subtitle">{{ Str::limit($blog->subtitle, 80) }}</p>
                </div>
            </a>
            @endforeach
        </div>
        <div style="text-align:center;margin-top:3rem;">
            <a href="{{ route('blog') }}" class="btn btn-outline">Read All Posts <i class="fas fa-arrow-right"></i></a>
        </div>
    </div>
</section>
@endif
    ...
    <div class="container">
        ...
        <div class="reviews-grid">
            ...
            <div class="review-card">
                ...
                <p class="review-text">"{{ Str::limit($review->description, 120) }}"</p>
                <div class="review-author">
                    <div class="review-avatar">{{ strtoupper(substr($review->user->username ?? 'U', 0, 1)) }}</div>
                    <div>
                        <div class="review-name">{{ $review->user->username ?? 'Customer' }}</div>
                        <div class="review-role">{{ $review->review_title }}</div>
                    </div>
                </div>
            </div>
            @endforeach
